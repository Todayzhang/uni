<template>
  <view>
    ...
    <button type="primary" @click="getBLEDeviceServices">
      获取蓝牙所有服务
    </button>
    <input type="text" v-model="blueData" placeholder="请输入要传递的值" />
    <button type="mini" @click="sendData">发送数据</button>
    <view
    ...
    >
      ...
      <button v-if="item.connect" class="concatBtn" type="primary">
        已连接
      </button>
      <button
        v-else
        class="concatBtn"
        type="primary"
        @click="createBLEConnection(item.name, item.address, index)"
      >
        连接
      </button>
    </view>
